...
function preload() {
  sourceImage = loadImage("./assets/images/image-12.jpg");
  font = loadFont("./assets/fonts-primary/Terminal-Grotesque.otf");
  metaFont = loadFont("./assets/fonts-primary/Activist.otf");
  loadFontFiles();
}
...
function setup() {
  ...
  poster = createGraphics(724, 1000);
  manipulateImage();
  busy = false; // gui.js
...
function buildUI() {
  ...
  <h2>Headline</h2>
    ...
    <div class="gui-wrapper">
      <div class="gui-label">Reset</div>
        <div class="gui-input">
         <button onclick="resetState()">Return to Default</button>
        </div>
        <div class="gui-val"></div>
      </div>
...
function databinding() {
  ...
  document.getElementById("fonts").querySelector("select").onchange = function () {
    ...
    font = loadFont("./assets/fonts-primary/" + newFont);
    console.log(newFont);
  }; // Font-size
...
function buildSelect(id, label, data) {
  ...
  var tmplt = `
	<div class="gui-wrapper" id="${id}">
      ...
      <select>
      <option style="position: absolute; right: 0" value="LOCKED" selected>LOCKED</option>
